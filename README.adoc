...
:consensusj-version: 0.7.0-beta1
:bitcoinj-version: 0.17.1
...
Before running `./gradlew` wrapper script you must have JDK 21 or later (JDK 25 is recommended) installed and your `JAVA_HOME` set correctly. To build native images you'll need a GraalVM JDK 25 or later and `GRAALVM_HOME` set correctly.
...
To run it and display a list of command line options:
    ...
    ./consensusj-jrpc/build/jrpc --help
