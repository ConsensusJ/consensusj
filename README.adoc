...
* A *Java* JSON-RPC client for  _any_ JSON-RPC server.
...
* `jrpc` - A JSON-RPC *command-line* client for testing and debugging JSON-RPC servers and applications. It can be natively compiled with https://www.graalvm.org[GraalVM]. It can launch, call the server and print output in _under 50 milliseconds_.
* `jsonrpc-echod` - A *Micronaut*-based JSON-RPC "echo" *server*. It can be natively compiled using https://www.graalvm.org[GraalVM].
...
|<<consensusj-jrpc,consensusj-jrpc>>
| *21*
| `org.consensusj.jrpc`
| The `jrpc` JSON-RPC command-line tool
...
| JSON-RPC command-line interface libraries
...
Before running `./gradlew` wrapper script you must have JDK 21 or later installed and your `JAVA_HOME` set correctly. To build native images you'll need a GraalVM JDK 21 or later and `GRAALVM_HOME` set correctly.
...
[#consensusj-jrpc]
=== Build `jrpc`, the JSON-RPC Command-Line Tool

To build the JSON-RPC CLI tool native image `jrpc`:

    ./gradlew :consensusj-jrpc:nativeCompile

To run it and display a list of command line options:

    ./consensusj-jrpc/build/jrpc -?
