...
Support for virtual currencies is one of the key design goals in the specification. This module will allow Bitcoin to
 be used by standard Java APIs in the same was as fiat currencies.

The JavaMoney Reference Implementation (aka "Moneta") contains ExchangeRateProviders for ECB (European Central Bank) and IMF (International Monetary Fund).
 There is also U.S. FRB (Federal Reserve Bank) and Yahoo Finance ExchangeRateProviders in the JavaMoney financial library add-on module.
