...
The JavaMoney Reference Implementation (aka "Moneta") contains `ExchangeRateProvider`s for ECB (European Central Bank) and IMF (International Monetary Fund).
 There is also U.S. FRB (Federal Reserve Bank) and Yahoo Finance `ExchangeRateProvider`s in the JavaMoney financial library add-on module.
