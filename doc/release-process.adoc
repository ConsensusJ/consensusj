...
. Use Java 25 for official builds
.. `sdk use 25.0.3-tem`
...
. Full build, test
.. `./gradlew clean buildCI consensusj-jrpc-echod:nativeTest nativeCompile installDist`
.. `./test-run-regtest.sh`
.. Recommended: test with *OmniJ* regTests.
.. `./scripts/build-release.sh`
. Tag: `git tag -a v0.x.y -m "Release 0.x.y"`
. Push: `git push --tags origin master`
. Set a GitLab token with Maven publishing permission: `export JRELEASER_GITLAB_CONSENSUSJ_TOKEN=xxx`
...
`nix-deps.json` holds SHA-256 hashes of Maven Central dependencies and is used by the `.#consensusj-tools` Nix flake. It currently must be manually updated whenever any Maven Central dependencies are updated.
...
At present, builds made with Temurin Java 25 (or equivalent) should be reproducible on
