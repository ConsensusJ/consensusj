...
* ByteBuddy 1.18.10
* Use Nixpkgs release-26.05
* Develocity 4.4.2

=== Build/Test Updates

* Use JReleaser to publish Maven Artifacts to GitLab
* Begin using `-DEV` for `consensusjVersion` instead of `-SNAPSHOT`
* Add `SECURITY.md` file
* Devshell used for testing now contains Bitcoin Core v31
* Update `test-run-regtest.sh` to support Bitcoin Core v31
* Remove x86_64-darwin support from `flake.nix`
...
* `TxOutSetInfo`: support both `hash_serialized_2` and `hash_serialized_3` (restores compatibility with earlier versions of Bitcoin Core) - https://github.com/ConsensusJ/consensusj/commit/d66f9f52f1b57ab6f1e8aca31e32b759aa58a14a[d66f9f5]

=== cj-btc-jsonrpc

* `RegTestFundingSource`: make `sendRawTransactionUnlimitedFees` compatible with both recent and older Bitcoin Core by setting `maxFeeRate` and `maxBurnAmount` to `null` - https://github.com/ConsensusJ/consensusj/commit/13b6e336f53f68d0ab3b122096dcead3e1557fd8[13b6e33]

=== Build/Test Updates

* Re-enable many regTest integration tests that had been disabled by https://github.com/ConsensusJ/consensusj/pull/228[PR 228] -- PRs https://github.com/ConsensusJ/consensusj/pull/350[350], https://github.com/ConsensusJ/consensusj/pull/351[351]
* Add `dumpPrivKeyAvailable = false` property in `gradle.properties` to more clearly mark which tests are disabled and why. If using older Bitcoin Core (or Omni Core) it can be set to `true` and more tests will be run.
* All regTests are verified working with Bitcoin Core v30.2 and Omni Core v0.12.0.1 (Bitcoin Core v20.1). Omni Core tests were run with `dumpPrivKeyAvailable` both `true` and `false`.
...
* Start using JSpecify with ErrorProne/NullAway checking in `jsonrpc`, `jpc`, `echod`, and `walletd` modules
* Improvements to `help` JSON-RPC method in multiple modules
* Remove `consensusj-decentralized-id`, `cj-eth-jsonrpc`,  `cj-nmc-jsonrpc` modules
* Add module-info to jsonrpc, jsonrpc-cli, jrpc, and related submodules

=== consensusj-jsonrpc

* `JsonRpcError`: add remaining predefined error codes
* add `JsonRpcNotification` class
* move `EchoJsonRpcWebService` from `echod` to `jsonrpc` (it's a web-framework independent service)

=== consensusj-jrpc

* New Gradle submodule using Gradle `application` plugin
* jsonrpc tool is now `jrpc` tool
* `jsonrpc` binary has been renamed `jrpc`

=== consensusj-jrpc-echod

* Renamed from `consensusj-jsonrpc-echod`
* Application binary renamed to `jrpc-echod`

=== consensusj-jsonrpc-cli

* `jsonrpc`/`jrpc` tool extracted to the `consensusj-jrpc` submodule
* This module is now strictly a library of CLI utilities

=== cj-btc-walletd

* Renamed from `cj-btc-daemon`
* Application binary renamed to `walletd`
...
* Only provide external Javadoc links in aggregated Javadoc (`javadocAll` Gradle task)
* Remove `javadocAll` Gradle task from `buildCI` task
* Migrate unit tests in `jrpc` and `jrpc-echod` to JUnit from Spock
* Implement `nativeTest` in `jrpc-echod` and run it on GitHub Actions
* Gradle 9.4.1
* Use Gradle Test Suite Plugin
* JUnit Jupiter 5.14.3
* ByteBuddy 1.18.8
* Objenesis 3.5
* Combine GitHub Actions into single `build.yml` workflow
* GitHub Actions: run `javadocAll` in a separate job.
* GitHub `gradle.yml` JDK Matrix: Java 21, 25
* git-publish Gradle Plugin 6.0.0
* Asciidoctor Gradle Plugin 4.0.5
* Develocity (aka Gradle Enterprise) 4.4.1
* Enable Dependabot weekly Nix flake updates
